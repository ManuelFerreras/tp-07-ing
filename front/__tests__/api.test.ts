...
test('deleteEmployee hits endpoint with delete verb', async () => {
  (global.fetch as jest.Mock).mockResolvedValueOnce({ ok: true, text: async () => '' } as Response);
  await deleteEmployee(7);
  expect(global.fetch).toHaveBeenCalledWith('http://api/employees/7', { method: 'DELETE' });
});
...
test('getReviews sends filter params', async () => {
  ...
  expect(global.fetch).toHaveBeenCalledWith('http://api/reviews?employeeId=7&state=draft', { cache: 'no-store' });
});

test('createReview trims data before sending', async () => {
  (global.fetch as jest.Mock).mockResolvedValueOnce(
    mockResponse({ id: 1, employeeId: 2, employeeName: 'A', period: '2024-Q4', reviewer: 'Lead', rating: 5, strengths: '', opportunities: '', state: 'draft' }, { status: 201 })
  );
  await createReview({
    employeeId: 2,
    period: ' 2024-Q4 ',
...
test('updateReview hits endpoint with payload', async () => {
  ...
  expect(global.fetch).toHaveBeenCalledWith('http://api/reviews/3', expect.any(Object));
});
...
test('getPayroll builds query string', async () => {
  (global.fetch as jest.Mock).mockResolvedValueOnce(
    mockResponse({ items: [], aggregates: { totalsByPeriod: [], grandTotalNet: 0 } })
  );
  await getPayroll({ employeeId: 1, period: '2024-10' });
  expect(global.fetch).toHaveBeenCalledWith('http://api/payroll?employeeId=1&period=2024-10', { cache: 'no-store' });
});
